...
import com.zhaoweihao.architechturesample.adapter.HomeCourseTaskTalkTopicCommentAdapter;
import com.zhaoweihao.architechturesample.contract.HomeCourseTaskTalkTopicCommentContract;
import com.zhaoweihao.architechturesample.presenter.HomeCourseTaskTalkTopicCommentPresenter;
...
/**
*@description 首页-课程-详细界面-任务-讨论-评论(学生评论)
*@author
*@time 2019/1/28 13:03
*/
public class HomeCourseTaskTalkTopicCommentActivity extends BaseActivity implements HomeCourseTaskTalkTopicCommentContract.View {
    ...
    private HomeCourseTaskTalkTopicCommentContract.Presenter presenter;
    private HomeCourseTaskTalkTopicCommentAdapter adapter;
    private String url;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new HomeCourseTaskTalkTopicCommentPresenter(this, this);
        initViews(null);
        Intent intent = getIntent();
        checkTecOrStu = presenter.checkTecOrStu();
        String suffix = Constant.QUERY_COMMENT_BY_DISCUSS_ID_URL +intent.getIntExtra("discussId",0);
...
//        url = suffix+"?"+"courseId="+intent.getIntExtra("courseId",0);
        ...
        ftbn_query_comment.setOnClickListener(v -> {
            ...
            builder.setPositiveButton("发送", (dialog, which) ->{
                String url1=Constant.ADD_COMMENT_URL;
                ...
                if(input.getText().toString().equals("")){
                    Toast.makeText(HomeCourseTaskTalkTopicCommentActivity.this,"请填写评论！",Toast.LENGTH_SHORT).show();
                }else {
    ...
    public void showResult(ArrayList<QueryComment> queryArrayList) {
        runOnUiThread(() -> {
            if (adapter == null) {
                adapter = new HomeCourseTaskTalkTopicCommentAdapter(this, queryArrayList,false);
    ...
    @Override
    public void setPresenter(HomeCourseTaskTalkTopicCommentContract.Presenter presenter) {
        if (presenter != null) {
